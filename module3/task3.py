...
class Company(object):
    ...
    def add_employee(self, employee):
        ...
        if (isinstance(employee, Engineer) or isinstance(employee, Manager)) and not employee.is_employed:
            self.employees.append(employee)
            employee.company = self
            print(f"Employee {employee.name} was added to the {self.name}")
        else:
            print(f"{employee} can't be employed")
    ...
    def dismiss_employee(self, employee):
        ...
        if employee in self.employees:
            ...
            print(f"Employee {employee.name} was dismissed from the {self.name}")
    ...
    def do_tasks(self, employee):
        ...
        salary_for_engineer = 10
        if employee in self.employees and isinstance(employee, Engineer):
            self.__money -= salary_for_engineer
            print(f"Employee {employee.name} has done task and got {salary_for_engineer}")
            return salary_for_engineer
    ...
    def write_reports(self, employee):
        ...
        salary_for_manager = 12
        if employee in self.employees and isinstance(employee, Manager):
            self.__money -= salary_for_manager
            print(f"Employee {employee.name} has done task and got {salary_for_manager}")
            return salary_for_manager
    ...
    def make_a_party(self):
        ...
        if not self.is_bankrupt and len(self.employees) * 5 < self.__money:
            print(f"{self.name} is organizing a party!")
            for employee in self.employees:
                employee.bonus_to_salary(self)
        else:
            print("It is not possible to organize a party now, because of not enough money in the budget")

    def show_money(self):
        """ Displays amount of money that company has """
        print(f"Current money for {self.name} is {self.__money}")
    ...
    def go_bankrupt(self):
        ...
        """
        self.__money = 0
        print(f"{self.name} is likely to be a bankrupt")
        for employee in self.employees:
...
class Employee(Person):
    ...
    def join_company(self, company):
        ...
        print(f"Employee {self.name} wants to work in a {company.name}")
        company.add_employee(self)

    def become_unemployed(self):
        """ Leave current company """
        if self.company is not None:
            print(f"Employee {self.name} wants to leave a {self.company.name}")
    ...
    def bonus_to_salary(self, company, reward=5):
        ...
        # money + 5
        if self.is_employed and self.company == company:
